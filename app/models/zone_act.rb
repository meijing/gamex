class ZoneAct < ActiveRecord::Base
  ...
  scope :by_zone, lambda { |zone_id, acts|
    joins(:erogenous_zone).where('(zone_acts.sex = erogenous_zones.sex or zone_acts.sex = 2) and zone_acts.erogenous_zone_id = ? and zone_acts.act_id in (?)', zone_id, acts) unless zone_id.nil?
  }
