class ZoneAct < ActiveRecord::Base
  ...
  scope :by_zone, lambda { |zone_id, acts, cont_rounds|
    joins(:erogenous_zone).where('(zone_acts.sex = erogenous_zones.sex or zone_acts.sex = 2) and zone_acts.erogenous_zone_id = ? and (? is null or zone_acts.act_id in (?)) and ((?<=20 and high_level=0) or ?>20)', zone_id, acts, acts, cont_rounds, cont_rounds) unless zone_id.nil?
  }
